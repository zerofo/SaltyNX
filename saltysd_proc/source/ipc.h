...
static inline bool serviceIsDomain_old(Service_old* s) {
    return s->type == ServiceType_Domain;
}

static inline bool serviceIsDomainSubservice_old(Service_old* s) {
 ...
 * @return Result code.
 */
static inline Result serviceIpcDispatch(Service_old* s) {
    return ipcDispatch(s->handle);
}

static inline Result serviceCloseObjectById(Service_old* s, u32 object_id) {
    return ipcCloseObjectById(s->handle, object_id);
}

static inline Result serviceIpcParse(Service_old* s, IpcParsedCommand* r, size_t sizeof_raw) {
 ...
 * @param[in] s Service object.
 */
static inline void serviceClose_old(Service_old* s) {
    switch (s->type) {
 ...
 */
static inline bool serviceIsActive_old(Service_old* s) {
 ...
 */
static inline void serviceCreate_old(Service_old* s, Handle h) {
    s->handle = h;
