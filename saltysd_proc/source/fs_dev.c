...
fsdev_devoptab =
{
  ...
  .mkdir_r      = fsdev_mkdir,
  .dirStateSize = sizeof(fsdev_dir_t_old),
...
static int _fsdevMountDevice(const char *name, FsFileSystem_old fs, fsdev_fsdevice **out_device)
{
  fsdev_fsdevice *device = NULL;
...
Result fsdevCommitDevice_old(const char *name)
{
  ...
  return fsFsCommit_old(&device->fs);
}
...
Result fsdevSetArchiveBit_old(const char *path) {
  ...
  return fsFsSetArchiveBit_old(&device->fs, fs_path);
}
...
Result fsdevCreateFile_old(const char* path, size_t size, int flags) {
  ...
  return fsFsCreateFile_old(&device->fs, fs_path, size, flags);
}
...
Result fsdevDeleteDirectoryRecursively_old(const char *path) {
  ...
  return fsFsDeleteDirectoryRecursively_old(&device->fs, fs_path);
}
          ...
          int           mode)
{
  ...
  if((flags & O_CREAT))
  {
    rc = fsFsCreateFile_old(&device->fs, fs_path, 0, attributes);
    if(flags & O_EXCL)
    {
      if(R_FAILED(rc))
      {
  ...
  /* open the file */
  rc = fsFsOpenFile_old(&device->fs, fs_path, fsdev_flags, &fd);
  if(R_SUCCEEDED(rc))
  {
    if((flags & O_ACCMODE) != O_RDONLY && (flags & O_TRUNC))
    {
      rc = fsFileSetSize_old(&fd, 0);
      if(R_FAILED(rc))
      {
        fsFileClose_old(&fd);
        r->_errno = fsdev_translate_error(rc);
    ...
    rc = fsFsGetFileTimeStampRaw_old(&device->fs, fs_path, &file->timestamps);//Result can be ignored since output is only set on success, etc.
           ...
           size_t        len)
{
  ...
  if(file->flags & O_APPEND)
  {
    ...
    rc = fsFileGetSize_old(&file->fd, &file->offset);
    if(R_FAILED(rc))
    {
      r->_errno = fsdev_translate_error(rc);
      return -1;
  ...
  rc = fsFileWrite_old(&file->fd, file->offset, ptr, len, FS_WRITEOPTION_NONE);
  if(rc == 0xD401)
                ...
                size_t        len)
{
  ...
  while(len > 0)
  {
    ...
    /* write the data */
    rc = fsFileWrite_old(&file->fd, file->offset, tmp_buffer, toWrite, FS_WRITEOPTION_NONE);
          ...
          size_t         len)
{
  ...
  /* read the data */
  rc = fsFileRead_old(&file->fd, file->offset, ptr, len, FS_READOPTION_NONE, &bytes);
  if(rc == 0xD401)
                ...
                size_t        len)
{
  ...
  while(len > 0)
  {
    ...
    /* read the data */
    rc = fsFileRead_old(&file->fd, file->offset, tmp_buffer, toRead, FS_READOPTION_NONE, &bytes);
          ...
          int           whence)
{
  ...
  switch(whence)
  {
    ...
    case SEEK_END:
      rc = fsFileGetSize_old(&file->fd, &offset);
      if(R_FAILED(rc))
      {
        r->_errno = fsdev_translate_error(rc);
        return -1;
           ...
           struct stat   *st)
{
  ...
  rc = fsFileGetSize_old(&file->fd, &size);
  if(R_SUCCEEDED(rc))
  {
          ...
          struct stat   *st)
{
  ...
  char    fs_path[FS_MAX_PATH];
  fsdev_fsdevice *device = NULL;
  FsTimeStampRaw_old timestamps = {0};
  FsEntryType_old type;

  if(fsdev_getfspath(r, file, &device, fs_path)==-1)
    return -1;

  rc = fsFsGetEntryType_old(&device->fs, fs_path, &type);
  if(R_SUCCEEDED(rc))
  {
    if(type == ENTRYTYPE_DIR)
    {
      if(R_SUCCEEDED(rc = fsFsOpenDirectory_old(&device->fs, fs_path, FS_DIROPEN_DIRECTORY | FS_DIROPEN_FILE, &fdir)))
      {
        memset(st, 0, sizeof(struct stat));
        st->st_nlink = 1;
    ...
    {
      if(R_SUCCEEDED(rc = fsFsOpenFile_old(&device->fs, fs_path, FS_OPEN_READ, &fd)))
      {
        ...
        if(ret==0)
        {
          rc = fsFsGetFileTimeStampRaw_old(&device->fs, fs_path, &timestamps);
            ...
            const char    *name)
{
  ...
  rc = fsFsDeleteFile_old(&device->fs, fs_path);
  if(R_SUCCEEDED(rc))
    return 0;
           ...
           const char    *name)
{
  ...
  rc = fsFsOpenDirectory_old(&device->fs, fs_path, FS_DIROPEN_DIRECTORY | FS_DIROPEN_FILE, &fd);
  if(R_SUCCEEDED(rc))
  {
    fsDirClose_old(&fd);
            ...
            const char    *newName)
{
  ...
  rc = fsFsGetEntryType_old(&device_old->fs, fs_path_old, &type);
  if(R_SUCCEEDED(rc))
  {
    if(type == ENTRYTYPE_DIR)
    {
      rc = fsFsRenameDirectory_old(&device_old->fs, fs_path_old, fs_path_new);
      if(R_SUCCEEDED(rc))
      return 0;
    }
    else if(type == ENTRYTYPE_FILE)
    {
      rc = fsFsRenameFile_old(&device_old->fs, fs_path_old, fs_path_new);
      if(R_SUCCEEDED(rc))
      return 0;
    }
    else
           ...
           int           mode)
{
  ...
  rc = fsFsCreateDirectory_old(&device->fs, fs_path);
  if(R_SUCCEEDED(rc))
    return 0;
             ...
             const char    *path)
{
  ...
  /* get pointer to our data */
  fsdev_dir_t_old *dir = (fsdev_dir_t_old*)(dirState->dirStruct);
             ...
             struct stat   *filestat)
{
  ...
  else
  {
    ...
    memset(dir->entry_data, 0, sizeof(dir->entry_data));
    rc = fsDirRead_old(&dir->fd, 0, &entries, max_entries, dir->entry_data);
    if(R_SUCCEEDED(rc))
    {
      if(entries == 0)
      {
             ...
             struct statvfs *buf)
{
  ...
  rc = fsFsGetFreeSpace_old(&device->fs, fs_path, &freespace);

  if(R_SUCCEEDED(rc))
    rc = fsFsGetTotalSpace_old(&device->fs, fs_path, &total_space);
               ...
               off_t         len)
{
  ...
  rc = fsFileSetSize_old(&file->fd, len);
  if(R_SUCCEEDED(rc))
    return 0;
           ...
           const char    *name)
{
  ...
  rc = fsFsDeleteDirectory_old(&device->fs, fs_path);
  if(R_SUCCEEDED(rc))
    return 0;
