...
Result jitTransitionToWritable(Jit* j)
{
    ...
    switch (j->type) {
    case JitType_CodeMemory:
        if (j->is_executable) rc = svcUnmapProcessCodeMemory(envGetOwnProcessHandle(), (u32) j->rx_addr, (u32) j->src_addr, j->size);
        break;
...
Result jitTransitionToExecutable(Jit* j)
{
    ...
    case JitType_CodeMemory:
        if (!j->is_executable) {
            rc = svcMapProcessCodeMemory(envGetOwnProcessHandle(), (u32) j->rx_addr, (u32) j->src_addr, j->size);

            if (R_SUCCEEDED(rc)) {
                rc = svcSetProcessMemoryPermission(envGetOwnProcessHandle(), (u32) j->rx_addr, j->size, Perm_Rx);
